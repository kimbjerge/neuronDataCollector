...
int Config::loadCoeffBin(string name) {
	...
	if (fileSize != sizeof(mCoeffsAll)) {
		printf(
				"Wrong size of binary coefficients file %s size %d expected 60 TABS in each 32 channels!!!\r\n",
				name.c_str(), fileSize);
		m_file.close();
		return -1;
	}
	...
	parseCoeff(); //print the filter coefficent //J
	mTabsValidAll = true;
...
void Config::parseConfig(void) {
	...
	mNumTemplates = 0;
	while (getNextLine()) {
		sscanf(mLineTxt, "%s %d %d %f %d %d %d %d %d %s", tempName, &width,
				&length, &threshold, &counter, &max, &min, &offset,
				&minGradient, tempCfg);
...
#else
		sscanf(mLineTxt, "%d %d %d %d %d", &value[0], &value[1], &value[2],
				&value[3], &value[4]);
#endif
		switch (lineNumber) {
