...
static void prvSetupHardware( void )
{
	...
	vConfigureHPPDataInterrupt((void *) pxGICConfig);
	vConfigureHPPAckInterrupt((void *) pxGICConfig);
	vConfigureHPPFileInterrupt((void *) pxGICConfig);
	...
	FreeRTOS_asm_vectors.S, which is part of this project.  Switch to use the
	FreeRTOS vector table. */
	//vPortInstallFreeRTOSVectorTable(); //KBE??? already done in freertos901_xilinx_bsp
}
/*-----------------------------------------------------------*/
...
{
const unsigned char *pucMem1 = (const unsigned char *)pvMem1, *pucMem2 = (const unsigned char *)pvMem2;
size_t x;
