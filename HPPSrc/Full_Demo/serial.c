...
/* The interrupt controller, which is configured by the hardware setup routines
defined in main(). */
...
XUartPs_Config *pxConfig;
	...
	/* Look up the UART configuration then initialise the driver. */
	pxConfig = XUartPs_LookupConfig( XPAR_XUARTPS_0_DEVICE_ID );

	/* Initialise the driver. */
	xStatus = XUartPs_CfgInitialize( &xUARTInstance, pxConfig, XPAR_PS7_UART_0_BASEADDR );
	...
	XUartPs_WriteReg( XPAR_PS7_UART_0_BASEADDR, XUARTPS_ISR_OFFSET, XUARTPS_IXR_MASK );
...
const TickType_t xMaxWait = 200UL / portTICK_PERIOD_MS;
	...
	XUartPs_Send( &xUARTInstance, ( u8 * ) pcString, usStringLength );
...
signed portBASE_TYPE xSerialPutChar( xComPortHandle pxPort, signed char cOutChar, TickType_t xBlockTime )
{
	...
	/* Send the character. */
	XUartPs_Send( &xUARTInstance, ( u8 * ) &cOutChar, sizeof( cOutChar ) );
...
char cChar;
	...
	ulActiveInterrupts = XUartPs_ReadReg(XPAR_PS7_UART_0_BASEADDR,  XUARTPS_IMR_OFFSET);
	ulActiveInterrupts &= XUartPs_ReadReg(XPAR_PS7_UART_0_BASEADDR,  XUARTPS_ISR_OFFSET);
	...
	if( ( ulActiveInterrupts & serTRANSMIT_IINTERRUPT_MASK ) != 0 )
	{
		if( xUARTInstance.SendBuffer.RemainingBytes == 0 )
		{
			...
			XUartPs_WriteReg( XPAR_PS7_UART_0_BASEADDR, XUARTPS_IDR_OFFSET, XUARTPS_IXR_TXEMPTY );
		}
		else
		{
			/* More data to send. */
	...
	XUartPs_WriteReg( XPAR_PS7_UART_0_BASEADDR, XUARTPS_ISR_OFFSET, ulActiveInterrupts );
}
